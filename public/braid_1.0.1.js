...
var braid = {
      ...
      success: function (f) {
        ...
        if (c.formats.indexOf('M') != -1) {
          ...
          e.onload = function () {
            c.display();
            a.image(
              c.border - c.minX * c.scale,
              c.border - c.minY * c.scale,
              this.width * c.scale,
              this.height * c.scale,
              c.url.slice(0, c.url.length - 3) + 'tm.png',
              {
                'pointer-events': 'none',
              }
            );
    ...
    if (this.oldAdjustables != null) {
      ...
      if (colourMaps != null && this.newMove == true) {
        for (e = 0; e < colourMaps.length; e++) {
          if (
            colourMaps[e].adjustableMap.from == f &&
            colourMaps[e].adjustableMap.to == h
          ) {
            b = e;
            break;
          }
          if (
            colourMaps[e].adjustableMap.to == f &&
            colourMaps[e].adjustableMap.from == h
          ) {
            g = e;
            break;
          }
  ...
  drawTakadai: function () {
    ...
    var c = this,
      ...
      l = $('#brCanvas').svg('get');
    n = new Array(
      this.startPosX - 150 + f * 12,
      this.startPosX - 150 + f * 9,
      this.startPosX - 150,
      this.startPosX - 150 + f * 3
    );
    n = new Array(
      this.startPosX - 150 + f * 12,
      this.startPosX - 150 + f * 9,
      ...
      for (g = 0; g < this.adjustables.length; g++) {
        ...
        $(
          ...
          function (i) {
            b.updateAdjustable(
              i.data.adjustable,
              0 - i.data.adjustable.increment
            );
          }
        );
        $('#ofo')
          .clone()
          .insertAfter($('#incr_' + this.adjustables[g].key))
          .attr({
            style: 'display:block',
    ...
    for (a = 0; a < this.adjustables.length; a++) {
      if (this.adjustables[a].key == b.name) {
        this.updateAdjustable(
          this.adjustables[a],
          $('#fo_' + this.adjustables[a].key + ' input:first').val() -
            this.adjustables[a].currentValue
        );
        break;
      }
    }
      ...
      if (b.length < 1) {
        ...
        $(a)
          ...
          .each(function () {
            ...
            if (
              $(this).attr('variables') != '' &&
              $(this).attr('variables').indexOf(h) == -1
            ) {
              $(this).addClass('hidden');
            }
          });
    ...
    if (b.length < 1) {
      ...
      $(a)
        ...
        .each(function () {
          ...
          if (
            $(this).attr('variables') != '' &&
            $(this).attr('variables').indexOf(j) == -1
          ) {
            $(this).addClass('hidden');
          }
        });
  ...
  checkButtons: function () {
    ...
    for (a = 0; a < this.adjustables.length; a++) {
      ...
      if (
        this.adjustables[a].currentValue >= this.adjustables[a].maxValue ||
        this.totalThreads(a, this.adjustables[a].increment) > this.maxThreads
      ) {
        $('#incr_' + this.adjustables[a].key).css({
          fill: '#cccccc',
          cursor: 'default',
